...
class PageController extends Controller
{
  ...
  public function contactForm(): InertiaResponse
  {
    ...
    private function getDashboardData(Request $request): array
    {
        ...
        $clicksHistory = $clicksQuery->select('label', DB::raw('count(*) as total'))->groupBy('label')->orderByDesc('total')->orderBy('label', 'asc')->get();
        $dbYears = DB::table('analytics_events')->select(DB::raw('YEAR(created_at) as year'))->distinct()->pluck('year');
        ...
        // --- BESUCHER-STATISTIKEN (überarbeitet) ---
        ...
        // NEU: Geräte-Filter aus dem Request holen
        ...
        // NEU: Filter anwenden, wenn er nicht 'all' ist
        if ($visitorDeviceType && $visitorDeviceType !== 'all') {
            $visitsQuery->where('device_type', $visitorDeviceType);
        }

        // NEU: Daten werden nach Datum UND Gerätetyp gruppiert
        $visitsHistoryRaw = $visitsQuery->select(
        ...
        // NEU: Rohdaten werden in ein für das Frontend optimiertes Format umgewandelt
        ...
        $dbVisitorYears = DB::table('visits')->select(DB::raw('YEAR(visited_at) as year'))->distinct()->pluck('year');
