...
onMounted(() => {
  setInterval(() => {
    axios.get(route('admin.analytics.data')).then(response => {
...
<template>
  <Card>
      ...
      <div class="flex flex-wrap justify-between items-start gap-4">
        <div>
          ...
          <p class="mt-2 text-2xl font-bold">
            {{ data?.totalToday || 0 }} <span class="text-sm font-normal text-muted-foreground">Klicks heute</span>
          </p>
        </div>
        <div class="flex flex-wrap items-center gap-2">
          <Select :model-value="localFilters.category_id" @update:modelValue="val => localFilters.category_id = val">
            <SelectTrigger class="w-[180px]"><SelectValue placeholder="Kategorie filtern" /></SelectTrigger>
            <SelectContent>
              <SelectItem value="all">Alle Kategorien</SelectItem>
              <SelectItem v-for="cat in data?.availableCategories || []" :key="cat.id" :value="cat.id.toString()">{{ cat.name }}</SelectItem>
            </SelectContent>
          </Select>
          <Select :model-value="localFilters.month" @update:modelValue="val => localFilters.month = val">
          ...
          <Select :model-value="localFilters.year" @update:modelValue="val => localFilters.year = val">
    ...
    <CardContent>
      <div class="h-[250px]">
        <Bar v-if="data?.history && data.history.length > 0" :data="chartData" :options="chartOptions" />
