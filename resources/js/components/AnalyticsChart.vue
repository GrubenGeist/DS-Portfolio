...
const emit = defineEmits<{
  (e: 'update:filters', payload: { year: string; month: string; category_id: string }): void;
}>();
...
const chartData = computed(() => {
  ...
  const labels = history.map((i) => i.label || 'Unbekannt');
  const data = history.map((i) => i.total);

  // Falls nur 1 Balken, etwas „Luft“ schaffen: Labels & Daten gemeinsam auffüllen
  if (labels.length === 1) {
    const padCount = 5;
  ...
  return {
    ...
    scales: {
      ...
      y: {
        ...
        title: { display: true, text: 'Anzahl der Klicks', color: textColor, font: { size: 14 } },
        grace: '10%', // etwas Luft oben
      },
    },
  };
});
      ...
      <div class="flex flex-wrap justify-between items-start gap-4">
        <div>
          ...
          <p class="mt-2 text-2xl font-bold">
            ...
            <span class="text-sm font-normal text-muted-foreground">Klicks heute</span>
          </p>
        </div>
    ...
    <CardContent>
      <div class="h-[250px]">
        ...
        <p v-else class="flex items-center justify-center h-full text-sm text-muted-foreground">
          Keine Klicks für diesen Zeitraum.
        </p>
      </div>
    </CardContent>
  </Card>
